import {User,Upvote,Tag,Thumbdown,Thumbup,Post,Comment} from './interfaces';
import { postsApi,commentsApi, upvoteApi,thumbupApi,thumbdownApi,viewsApi,deleteThumbsUpApi,deleteThumbsDownApi } from './env_variables';

export const getAuthorName=(postId:number,users:{name:string,posts:{id:number}[]}[]):string|undefined=>{
 ...
 export const getAuthorAvatar=(postId:number,users:{posts:{id:number}[],image:string}[]):string|undefined=>{
    for (let user of users){
      for(let post of user.posts){
        if(post.id==postId){
          return user.image
  ...
  export const getTeaserText=(text:string, windowSize:number):string=>{
    ...
    return text.length>maxLengthOfTeaserText? text.slice(0,maxLengthOfTeaserText)+'...':text
...
export const postAlreadyUpvotedByUser=(upvotes:Thumbup[],user:string):boolean=>{
...
export const commentAlreadyVotedByUser=(votes:Thumbup[],user:string,commentId:number):boolean=>{
...
export const getSinglePost=(posts:Post[],postId:number):Post|undefined=>{
...
export const getUserPostsQuantity=(users:User[],authorName:string):number|undefined=>{
  for (let user of users){
...
export const handleSubmitVote=async(id:number,user:string,upvotes:Upvote[])=>{
...
export const addPostToDb = async (title:string, content:string,author:User,tags:string[]) => {
...
export const getCommentsIdsUnlikedByUser=(comments:Comment[],userEmail:string):number[]=>{
  const unlikes=[];
...
export const getCommentsIdsLikedByUser=(comments:Comment[],userEmail:string):number[]=>{
  const likes=[];
...
export const checkIfAlreadyLiked=(commentsIdsLikedByUser:number[],commentId:number):boolean=>{
...
export const checkIfAlreadyUnliked=(commentsIdsUnlikedByUser:number[],commentId:number):boolean=>{
...
export const removeThumbUp=async (commentId:number,userEmail:string,refresh:()=>void)=>{
...
export const removeThumbDown=async (commentId:number,userEmail:string,refresh:()=>void)=>{
...
export  const handleUpdateThumbsUp=async(commentId:number,user:string,refresh:()=>void)=>{
...
export const handleUpdateThumbsDown=async(commentId:number,user:string,refresh:()=>void)=>{
