...
import { getAuthorName,getTeaserText,getAuthorAvatar,getTags,findPostsByTag,jsonFetch,handleIncreaseViews} from "../../helperFunctions";
...
const PostsSelectedByTagName:NextPage<PostsSelectedByTagNameProps>=({allPosts,users,postsIdsByTag,tagName})=>{
    ...
    return (
        ...
        <Sidebar itemSelected={"tags"} value={searchedWord} onChange={(e)=>setSearchedWord((e.target as HTMLInputElement).value)}
        ...
        allPosts.map(post=>{
            for(let postId of postsIdsByTag){
                if(postId.id==post.id){
                    return   <QuestionListItem key={post.id} nickname={getAuthorName(post.id,users)}
                    date={post.createdAt} title={post.title} text={getTeaserText(post.content,windowSize)} imageUrl={getAuthorAvatar(post.id,users)} tags={getTags(post.tags)} 
